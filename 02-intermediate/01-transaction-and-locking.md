...
MVCC의 대가로 **오래된 행 버전(dead tuple)** 이 쌓이므로, PostgreSQL은 `VACUUM`으로, Oracle은 UNDO 세그먼트로 이를 정리·관리한다. 오래 열려 있는 트랜잭션은 정리를 방해하므로 주의해야 한다.
...
-- 공유 락으로 읽기 (표준 구문)
SELECT * FROM accounts WHERE id = 100 FOR SHARE;        -- PostgreSQL
