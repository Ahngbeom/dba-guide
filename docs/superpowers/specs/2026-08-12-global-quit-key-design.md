...
**알려진 한계(고치지 않고 기록한다)**: `PAGER=cat`처럼 대체 화면을 쓰지 않는 페이저를
지정하면 본문이 터미널에 그대로 남는데도 `printed_inline`은 `False`가 되어 pause를 건너뛰고,
다음 curses 프레임이 그 본문을 지운다. 페이저가 대체 화면을 쓰는지 알아낼 이식성 있는 방법이
없다. `COLOR_PAGERS` 목록을 재사용하는 것도 답이 아니다 — 그 목록은 "ANSI를 통과시키는가"를
뜻하지 "화면을 복원하는가"를 뜻하지 않으며, 두 성질은 우연히 겹칠 뿐이다.
