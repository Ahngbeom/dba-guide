...
알려주지 않는다. 그런데 **길이**는 알려준다 — 이 저장소를 측정했을 때 85문항 중
67개(79%)에서 정답이 가장 긴 보기였고, 정답 평균 37자 대 오답 21자였다. "항상 가장
긴 것을 찍는다"만으로 79%가 나오는데 통과 기준이 70%라, 한 문제도 읽지 않고
객관식을 통과할 수 있었다.
...
진단 문항은 `tests/test_shooting.py`의 `DiagnosisChoiceLengthTest`). 기존 빚은
기준선으로 남겨 두었으니 **늘리지 말고 줄여라** — 은행을 고쳤으면 그 줄의 숫자도
함께 낮춘다. 새로 만든 은행은 기준선이 없어 절반이 상한이다.
