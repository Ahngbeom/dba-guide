...
수 있었다. **지금은 85문항 전부에서 0**이고 비율은 1.10이다.
...
보기가 **제품명이나 정해진 용어**라 길이를 맞출 수 없어 보이면, 정식 명칭으로
통일하거나 네 보기 모두에 같은 형식의 뜻풀이를 붙여라 — `pg_dump` 문항은
`MySQL`을 `MySQL / MariaDB`로, 이상 현상 문항은 네 보기 모두 `영문명(한글 뜻)`
형식으로 맞췄다. 한쪽에만 뜻풀이를 붙이면 **그 형식 자체가 새 단서**가 된다.

`tests/test_exam.py`의 `AnswerLeakLintTest`가 이것을 강제한다(스테이지 진단
문항은 `tests/test_shooting.py`의 `DiagnosisChoiceLengthTest`). 정답이 다른 어떤
보기보다 길면 **그 은행은 실패한다.** 동점은 세지 않는다 — 같은 길이의 오답이
있으면 "가장 긴 것"을 고를 수 없기 때문이고, 위의 뒤집기 기법은 의도적으로
동점을 만든다. 어쩔 수 없는 은행이 생기면 테스트의 기준선 표에 **사유와 함께**
적되, 지금은 비어 있다.
