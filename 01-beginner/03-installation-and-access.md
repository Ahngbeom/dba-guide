...
```bash
# 1) 서비스 시작
brew services start mysql               # (Linux면 sudo systemctl start mysql)

# 2) 서비스가 잘 떴는지 상태 확인
brew services list                      # mysql 항목이 started인지 확인
...
```sql
-- 4) 접속되면 프롬프트가 mysql> 로 바뀐다. 아래를 실행해 보자.
SHOW DATABASES;    -- 어떤 데이터베이스들이 있는지 확인
