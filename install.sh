...
옵션 없이 실행하면 설치하고, 이미 설치돼 있으면 업데이트한다(멱등).
  ...
  --uninstall --purge  저장소 디렉터리까지 지운다. 지우기 전에 확인을 받으므로
                       터미널이 필요하다 — curl … | bash 파이프에서는 표준 입력이
                       curl이라 답을 받을 수 없어 거부한다. 스크립트를 내려받아
                       bash install.sh --uninstall --purge 로 실행한다.
  --help               이 도움말.
