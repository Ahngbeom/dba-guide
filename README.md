...
DB 접근 방법은 두 가지다. **`c` 키로 게임 화면 안에서 바로 SQL 콘솔**을 열 수 있고(상단에 목표 요약줄이 남아 문맥을 잃지 않으며, 콘솔이 열린 동안에도 감시가 계속 돌아 그 자리에서 클리어된다), 원한다면 **외부 터미널**로 접속해도 된다. 접속 명령은 게임 화면 상단에 항상 떠 있다. **판정은 어느 쪽을 쓰든 완전히 동일하다** — 판정이 보는 것은 어디서 쳤는지가 아니라 `general_log`에 누구로 기록됐는지이기 때문이다. (콘솔은 질의마다 새 접속이라 트랜잭션 유지·`SET SESSION`은 외부 터미널을 쓴다.)

조작키는 플레이 화면에서 `c` SQL 콘솔 · `r` 상황 보고 · `h` 힌트 · `q` 포기이고, 문항에서는 `↑`·`↓`·숫자로 고르고 `Enter`로 제출한다. 콘솔에서는 `Tab`으로 스키마·테이블 이름을 완성하고(MySQL은 존재하지 않는 객체도 권한 오류로 답해 오타를 권한 문제로 착각하게 만든다), 결과가 화면보다 넓으면 `Ctrl+F`/`Ctrl+B`로 밀거나 질의 끝에 `\G`를 붙여 세로로 본다. **`Esc`를 누르면 제출하지 않고 닫히며**(고르던 위치와 입력 내용은 보존된다) `r`로 다시 열어 이어서 답할 수 있다. 화면 하단의 `⠹ 감시 중 · 4초 전 갱신`은 감시가 살아있는지와 데이터 신선도를 보여주고, 문제가 생기면 빨간 `⚠ 감시 오류`가 뜬다.
