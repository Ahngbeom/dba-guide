...
설치에 성공하면 그 위치가 `${XDG_STATE_HOME:-~/.local/state}/dba-guide/install-path`에
기록된다. 그래서 이후 업데이트·제거에는 값을 다시 주지 않아도 되고, 기록을 쓸 때는
어느 쪽을 골랐는지 알려 준다. 옮기고 싶으면 새 값을 주고 다시 설치하면 된다 — 옛
설치본은 지워지지 않으니 학습 기록은 직접 옮기거나 정리한다.

> **`v1.3.0` 이전에 커스텀 경로로 설치했다면** 기록이 없다. 다음 한 번만 `XDG_DATA_HOME`을
> 함께 주면 그때 기록되고, 그 뒤로는 생략해도 된다. 그냥 생략하면 기본 경로에 설치본이
> 하나 더 생기므로, 스크립트가 그 사실을 알려 준다.
